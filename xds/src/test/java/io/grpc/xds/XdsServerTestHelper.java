...
public class XdsServerTestHelper {
  ...
  static final class FakeXdsClient extends XdsClient {
    ...
    void deliverLdsResourceNotFound() {
      execute(() -> ldsWatcher.onResourceChanged(StatusOr.fromStatus(Status.NOT_FOUND.withDescription("LDS resource not found"))));
    }

    void deliverRdsUpdate(String resourceName, List<VirtualHost> virtualHosts) {
      execute(() -> rdsWatchers.get(resourceName).onResourceChanged(StatusOr.fromValue(new RdsUpdate(virtualHosts))));
    }
    ...
    void deliverRdsResourceNotFound(String resourceName) {
      execute(() -> rdsWatchers.get(resourceName).onResourceChanged(StatusOr.fromStatus(Status.NOT_FOUND.withDescription("RDS resource not found: " + resourceName))));
    }
  }
}
